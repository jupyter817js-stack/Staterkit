...
export default function StoresPageHeader({ onRefresh, onAdd, loading }: StoresPageHeaderProps) {
  const { t } = useLanguage();
  return (
    <div className="md:flex block items-center justify-between my-[1.5rem] page-header-breadcrumb">
      <div className="flex items-start gap-3">
        <span className="!w-10 !h-10 !rounded-lg inline-flex items-center justify-center bg-primary/10 text-primary flex-shrink-0">
          <i className="ri-store-2-line text-[1.25rem]" />
        </span>
        <div>
      ...
      <div className="btn-list md:mt-0 mt-3 flex flex-wrap gap-2">
        ...
        {onAdd && (
          ...
          >
            ...
            <span className="ms-1.5">{t("addStore")}</span>
          </button>
        )}
      </div>
    </div>
