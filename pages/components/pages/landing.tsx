...
import { createSubscription, getSubscriptionPaymentUrl } from '@/shared/api/nowpayments';
import { SUBSCRIPTION_PAYMENT_PENDING_KEY } from '@/shared/constants/subscription-payment';
...
const Landing = ({ local_varaiable, ThemeChanger }: any) => {
    ...
    const [pricingSubmitting, setPricingSubmitting] = useState<'STANDARD' | 'PRO' | null>(null);
    const [pricingError, setPricingError] = useState<string | null>(null);
    const [pricingSuccess, setPricingSuccess] = useState<string | null>(null);
    const [pricingEmailSentTo, setPricingEmailSentTo] = useState<string | null>(null);
    ...
    useEffect(() => {
        getCurrentUser().then((me) => setCurrentUser(me ?? null));
    }, []);
    ...
    const handlePricingSubscribe = useCallback(async (plan: 'STANDARD' | 'PRO') => {
        ...
        setPricingSubmitting(plan);
        try {
            const res = await createSubscription({ plan, email });
            const paymentUrl = getSubscriptionPaymentUrl(res);
            if (paymentUrl && typeof window !== 'undefined') {
                sessionStorage.setItem(SUBSCRIPTION_PAYMENT_PENDING_KEY, Date.now().toString());
                window.open(paymentUrl, '_blank', 'noopener,noreferrer');
                setPricingSuccess(t('landing_paymentPageOpened'));
            } else {
                setPricingSuccess(t('landing_paymentLinkSent'));
                setPricingEmailSentTo(email);
            }
        } catch (e) {
            setPricingError(e instanceof Error ? e.message : t('landing_subscriptionFailed'));
        } finally {
            setPricingSubmitting(null);
        }
    }, [router, t]);
                ...
                <div className="main-content !p-0 landing-main dark:text-defaulttextcolor/70 overflow-x-hidden" onClick={() => handleClick()}>
                    ...
                    <section className="section landing-section-pricing text-defaulttextcolor dark:text-defaulttextcolor/70 text-[0.813rem]" id="pricing">
                        <div className="container-xl px-3 text-center">
                            ...
                            {pricingError && (
                                <div className="alert alert-danger mb-4 flex rounded-lg max-w-2xl mx-auto" role="alert">
                                    <i className="ri-error-warning-line me-2 text-[1.125rem]" />
                                    <span className="text-[0.875rem]">{pricingError}</span>
                                </div>
                            )}
                            {pricingSuccess && (
                                <div className="alert alert-success mb-4 flex flex-col rounded-lg max-w-2xl mx-auto text-left" role="alert">
                                    <div className="flex items-start">
                                        <i className="ri-checkbox-circle-line me-2 text-[1.125rem] flex-shrink-0 mt-0.5" />
                                        <div className="text-[0.875rem]">
                                            <p className="mb-0">{pricingSuccess}</p>
                                            {pricingEmailSentTo && (
                                                <p className="mb-0 mt-1.5 text-defaulttextcolor/80 dark:text-white/70 text-[0.8125rem]">
                                                    {t('landing_paymentLinkSentTo')} <span className="font-medium">{pricingEmailSentTo}</span>
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                                ...
                                                <div className="xxl:col-span-4 xl:col-span-4 lg:col-span-4 md:col-span-4 sm:col-span-12 col-span-12  md:border-e md:border-b-0 border-b border-dashed dark:border-defaultborder/10 flex flex-col">
                                                    <div className="p-6 flex flex-col flex-1 min-h-0">
                                                        ...
                                                        <div className="py-6 flex items-center justify-center">
                                                            ...
                                                            <div className="text-end ms-[3rem]">
                                                                <p className="text-[1.5625rem] font-semibold mb-0">$89</p>
                                                                <p className="text-[#8c9097] dark:text-white/50 text-[0.6875rem] font-semibold mb-0">{t('landing_perMonth')}</p>
                                                            </div>
                                                        </div>
                                                        <ul className="list-unstyled text-center text-[0.75rem] px-4 pt-4 mb-0">
                                                        ...
                                                        <div className="grid mt-auto pt-2">
                                                            {currentUser?.subscription_plan === SUBSCRIPTION_PLAN.STANDARD ? (
                                                                ...
                                                                <button type="button" className="ti-btn ti-btn-primary !font-medium" disabled={!!pricingSubmitting} onClick={() => handlePricingSubscribe(SUBSCRIPTION_PLAN.STANDARD)}>
                                                                    {pricingSubmitting === SUBSCRIPTION_PLAN.STANDARD ? <span className="ti-spinner !w-5 !h-5 !border-2 inline-block" /> : t('landing_getStarted')}
                                                                </button>
                                                            )}
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="xxl:col-span-4 xl:col-span-4 lg:col-span-4 md:col-span-4 sm:col-span-12 col-span-12 flex flex-col">
                                                    <div className="p-6 pricing-offer overflow-hidden flex flex-col flex-1 min-h-0">
                                                        ...
                                                        <div className="py-6 flex items-center justify-center">
                                                            ...
                                                            <div className="text-end ms-[3rem]">
                                                                <p className="text-[1.5625rem] font-semibold mb-0 !text-primary">$199</p>
                                                                <p className="text-[#8c9097] dark:text-white/50 text-[0.6875rem] font-semibold mb-0">{t('landing_perMonth')}</p>
                                                            </div>
                                                        </div>
                                                        <ul className="list-unstyled text-center text-[0.75rem] px-4 pt-4 mb-0">
                                                        ...
                                                        <div className="grid mt-auto pt-2">
                                                            {currentUser?.subscription_plan === SUBSCRIPTION_PLAN.PRO ? (
                                                                ...
                                                                <button type="button" className="ti-btn ti-btn-primary !font-medium" disabled={!!pricingSubmitting} onClick={() => handlePricingSubscribe(SUBSCRIPTION_PLAN.PRO)}>
                                                                    {pricingSubmitting === SUBSCRIPTION_PLAN.PRO ? <span className="ti-spinner !w-5 !h-5 !border-2 inline-block" /> : t('landing_getStarted')}
                                                                </button>
                                                            )}
                                                        </div>
                                                    </div>
